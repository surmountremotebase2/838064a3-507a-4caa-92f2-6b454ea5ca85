...
Corrected version:
- Always emits explicit target weights.
- Uses BIL as an idle-capital sleeve.
- Prevents unintended full-SPY allocation when no active signal exists.
- Keeps total target allocation at 100%:
    * Active trade allocation: up to MAX_TOTAL_RISK_ALLOCATION.
    * Remaining allocation: BIL.
- Starts in conservative bull-trend-only mode.

Important:
- BIL is a short-duration Treasury ETF, not literal cash.
- This file cannot guarantee profit or prevent losses.
- ATHENA must enforce live stops, targets, spread checks, PMC validation,
  broker reconciliation, time stops, and forced EOD flatten.
"""
...
# ============================================================================
# ALLOCATION / CAPITAL-SLEEVE SETTINGS
# ============================================================================

# Surmount target weights should total 1.00.
# BIL receives all capital not allocated to an active trade.
CASH_SYMBOL = "BIL"
...
# Allocation proxy, not broker-dollar risk.
RISK_PER_TRADE = 0.0015

# Do not trade if the modeled favorable move is too small to clear friction.
...
# Assumes Surmount OHLCV timestamps are US/Eastern.
# Verify this with Test A/B/C before relying on time gates.
...
def build_universe():
    """
    BIL is included unconditionally because it is the explicit idle-capital
    sleeve. It must be inside `assets` if it appears in TargetAllocation.
    """
    symbols = [
        "SPY",
        "QQQ",
...
def rsi(values, length=RSI_PERIOD):
    ...
    average_gain = sum(max(value, 0.0) for value in changes) / float(length)
    average_loss = sum(max(-value, 0.0) for value in changes) / float(length)
...
def candle_pressure_proxy(bars, length=5):
    """
    OHLCV candle/volume proxy only.
    This is not true order flow, market depth, bid/ask delta, or PMC.
    """
...
def session_groups(bars):
    """
    Groups bars by calendar date.

    Confirm whether Surmount timestamps are UTC or US/Eastern before using this
    in a production intraday workflow.
    """
    groups = {}
    order = []
...
def session_rvol(bars, lookback_sessions=RVOL_LOOKBACK_SESSIONS):
    """
    Compare latest bar volume with the same intraday slot on prior sessions.
    Uses 20-bar fallback only if there is insufficient session history.
    """
    grouped = session_groups(bars)
...
def reference_levels(bars):
    ...
    if orb30:
        ...
        if not any(value is None for value in highs30 + lows30):
            output["orb30_high"] = max(highs30)
            output["orb30_low"] = min(lows30)

    return output


# ============================================================================
# CLASSIFICATION / ALLOCATION HELPERS
# ============================================================================
...
def estimated_round_trip_cost(symbol):
    """
    Conservative round-trip cost floor.

    Replace with actual commissions, spread, and slippage learned from ATHENA
    paper fills before considering live deployment.
    """
...
def add_cash_sleeve(risk_allocations):
    """
    Force all portfolio capital to have a stated target.

    Example:
        {"SPY": 0.15} becomes {"SPY": 0.15, "BIL": 0.85}

    This avoids relying on platform-specific interpretation of an empty or
    under-allocated TargetAllocation.
    """
    active = {
        str(symbol).upper(): float(weight)
        for symbol, weight in risk_allocations.items()
        if weight is not None and float(weight) > 0
    }

    active.pop(CASH_SYMBOL, None)

    total_active = sum(active.values())

    # Defensive normalization: no output may exceed 100% total allocation.
    if total_active > 1.0:
    ...
    if USE_BIL_CASH_SLEEVE:
        active[CASH_SYMBOL] = max(0.0, 1.0 - total_active)
...
class TradingStrategy(Strategy):
    """
    Stateless Surmount allocation strategy.

    All idle capital receives an explicit BIL target allocation. ATHENA remains
    responsible for real position state and actual broker-order lifecycle.
    """
    ...
    def candidate(self, symbol, data, context, phase):
        ...
        # --------------------------------------------------------------------
        # BULL TREND: ORB / VWAP RECLAIM / MOMENTUM
        # --------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------
        # BEAR TREND: EXPLICIT INVERSE ETF CONTINUATION
        # --------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------
        # FINAL SAFETY AND ECONOMIC-EDGE GATES
        # --------------------------------------------------------------------
    ...
    def allocation_for(self, candidate, regime):
        """
        Calculate target weight for one qualified candidate.

        This is a risk proxy. ATHENA must convert it into broker-safe quantity
        based on account value, open positions, actual stop, and fill quality.
        """
        risk_distance = max(
        ...
        return clamp(weight, 0.0, maximum)

    def run(self, data):
        """
        Surmount strategy entry point.

        The function always returns an explicit allocation:
        - No valid signal -> 100% BIL.
        - Valid signal -> active allocation + residual BIL.
        """
        try:
            phase = self.phase(data)

            # Explicit BIL target replaces ambiguous empty allocation.
            if phase in {
                "UNKNOWN",
                "PRE_OPEN",
                "CLOSE_WINDOW",
            ...
            if total_risk > MAX_TOTAL_RISK_ALLOCATION and total_risk > 0:
                scale = MAX_TOTAL_RISK_ALLOCATION / total_risk

                risk_allocations = {
                    symbol: allocation * scale
                    for symbol, allocation in risk_allocations.items()
                }

            return TargetAllocation(
                add_cash_sleeve(risk_allocations)
            )

        except Exception:
            # Fails closed to the explicit idle-capital sleeve.
            return TargetAllocation(add_cash_sleeve({}))